        ...
        <section class="mt-9">
            ...
            <div class="mt-4 grid grid-cols-2 gap-y-5 sm:grid-cols-3 gap-x-5">
                ...
                <div class="flex flex-col h-full rounded-xl overflow-hidden border dark:border-zinc-600 shadow-lg">
                    ...
                    {{-- Saya ubah menjadi flex-grow agar tombol favorit bisa didorong ke bawah jika perlu --}}
                    <div class="bg-white dark:bg-zinc-800 dark:text-white px-3 py-2 flex flex-col justify-between flex-grow border-t-2 border-t-red-600">
                        {{-- Baris untuk Judul dan Skor IMDb --}}
                        <div class="flex items-start justify-between w-full mb-1">
                            <span class="capitalize font-semibold text-sm truncate block" title="{{ $movie->title }}">{{ $movie->title }}</span>
                            <div class="flex space-x-1 items-center text-xs ml-2 shrink-0">
                                <svg class="w-7 h-4" xmlns="http://www.w3.org/2000/svg" width="64" height="32" viewBox="0 0 64 32" version="1.1">
