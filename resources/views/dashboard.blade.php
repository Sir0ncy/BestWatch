            ...
            <div class="mt-4 grid grid-cols-2 gap-y-5 sm:grid-cols-3 gap-x-5">
                @forelse ($movies as $movie)
                    <a href="{{ route('movies.show', $movie) }}"
                        class="flex flex-col h-full rounded-xl overflow-hidden border dark:border-zinc-600 hover:shadow-lg transition duration-300">
                        ...
                        <div class="absolute top-2 right-2 z-10">
                            ...
                            <form action="{{ route('my-list.remove', $movie->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" title="Hapus dari Favorit" class="text-yellow-400 hover:text-yellow-500">
                            ...
                            <form action="{{ route('my-list.add', $movie->id) }}" method="POST">
                                @csrf
                                <button type="submit" title="Tambah ke Favorit" class="text-gray-300 hover:text-yellow-400">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-6 h-6" viewBox="0 0 24 24">
                                        <path d="M12 .587l3.668 7.431 8.207 1.193-5.938 5.787 1.402 8.173L12 18.896l-7.339 3.875 1.402-8.173-5.938-5.787 8.207-1.193z"/>
                                    </svg>
                                </button>
                        ...
                        <div
                            ...
                            <div class="flex space-x-2 items-center text-xs"><span
                                    class="capitalize font-medium truncate">{{ $movie->title }}</span>
